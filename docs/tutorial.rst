...
No headers have been set, so we get a list of tuples (rows).
...
This part of the documentation serves to give you an idea of possiblities that
are otherwise hard to extract from the :ref:`API Documentation <api>`.
...
When you add a dynamic column, the first argument that is passed in to the
given callable is the current data row.
You can use this to perform calculations on your data row.
...
it's quite common to group multiple spreadsheets into a single Excel file,
known as a Workbook.
...
The resulting ``students.xls`` file will contain a separate spreadsheet for
each :class:`Dataset` object in the :class:`Databook`.
...
it's often useful to create a blank row containing information on the upcoming data.
So,
