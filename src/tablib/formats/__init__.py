    ...
    def register(self, key, format_):
        ...
        try:
            setattr(
                Databook,
                format_.title,
                property(format_.export_book, format_.import_book),
            )
        ...
        try:
            try:
                setattr(
                    Dataset,
                    format_.title,
                    property(format_.export_set, format_.import_set),
                )
                setattr(
                    Dataset,
                    "get_%s" % format_.title,
                    partialmethod(Dataset._get_in_format, format_),
                )
                setattr(
                    Dataset,
                    "set_%s" % format_.title,
                    partialmethod(Dataset._set_in_format, format_),
                )
            except AttributeError:
            ...
            raise Exception(
                "Your format class should minimally implement the export_set interface."
            )
